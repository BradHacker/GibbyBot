...
client.on('ready', () => {
  ...
  let channel = client.channels.cache.find(
    (channel) => channel.name === (preferences.testMode ? 'gibby-test' : preferences.mainChannel),
  );
  if (channel) {
    shouldSleep(channel);
...
const shouldSleep = (channel) => {
  ...
  if (hour >= 3 && hour < 11) {
    ...
    if (!sleep.isSleeping) {
      ...
      if (!sleep.sentSleepMessage)
        channel
          .send(preferences.battleMode ? 'I shall not sleep, for we are still at war' : preferences.sleepMessage)
  ...
  if (hour >= 11 || hour < 3) {
    client.user
      .setPresence({ activity: { type: 'WATCHING', name: 'for your merch order' }, status: 'online' })
      .then(() => '');
    if (sleep.isSleeping) {
      sleep.isSleeping = false;
      sleep.sentSleepMessage = false;
      if (!sleep.sentAwakeMessage)
        channel
          .send(preferences.battleMode ? "Good morning Giblets, reminder we're still at war" : preferences.awakeMessage)
          .catch((err) => console.error(err));
      sleep.sentAwakeMessage = true;
      fs.writeFile('sleep.json', JSON.stringify(sleep, null, 2), (err) => {
        if (err) console.error("couldn't save sleep data");
...
Discord.GuildMember.prototype.isAdmin = function () {
  // console.log(this)
  if (this.roles.cache.find((r) => preferences.adminRoles.filter((ar) => ar === r.name).length > 0)) return true;
  return false;
};
...
client.on('guildMemberAdd', (member) => {
  ...
  let channel = client.channels.cache.find((channel) => channel.name === preferences.mainChannel);
  if (!channel) return;
  channel.send(`**${member.user.username}**, ${preferences.welcomeMessage}`);
});
...
client.on('message', (msg) => {
  ...
  if (preferences.excludedChannels.filter((c) => c === msg.channel.name).length > 0) return;
  ...
  if (Utils.isOnNoNoList(msg.author)) {
    userData[msg.author.id].messagesSent++;
    messageBatchTotal++;
  }

  if (msg.channel.name === 'trading-bot' && !/gibby/i.test(msg.toString()) && !/trading-post/i.test(msg.toString()))
    return msg.delete().then(null, (err) => console.error(err));

  if (/bucket/i.test(msg.toString()) && !sleep.isSleeping) msg.react('🥣');

  if (preferences.battleMode && msg.member.roles.cache.get('750526846990549022')) {
    if (/gib/i.test(msg.toString())) {
      return msg.reply('silence non-gibson scum').then(() =>
        msg
          .delete()
          .then(() => console.log('non-scum has been vanquished'))
          .catch(console.error),
      );
    }
    if (Math.random() < 0.05) {
      ...
      return fetch('https://www.rappad.co/api/battles/random_insult')
        .then((response) => response.json())
        .then((insultData) => msg.reply(insultData.insult));
    }
    if (/sol/i.test(msg.toString())) {
      return msg.reply('sol sucks eggs').then(() =>
        msg
          .delete()
          .then(() => console.log('sol sucks eggs'))
          .catch(console.error),
      );
    }
  }
  ...
  if (/(valentine)|(be my valentine)/i.test(msg.toString()) && Math.random() < 0.7) {
    if (Math.random() < 0.1) {
      ...
      if (identifyInsult < 0.15)
        return msg.reply('What would you call someone who goes out with you? Desperate... *mic drop*');
      else if (identifyInsult < 0.3) return msg.reply("It's okay, your going to die alone anyways...");
      else if (identifyInsult < 0.45)
        return msg.reply('Where have you been all of my life? Can you go back there? Thanks...');
      else if (identifyInsult < 0.6)
        return msg.reply("Roses are Red\nCandy is sweet\nYou'll fit in my freezer\nIf I cut off your feet");
      else if (identifyInsult < 0.75) return msg.reply('They will never find your body...');
      else if (identifyInsult < 0.9) return msg.reply('I bet you hold hands with yourself... loser');
      else return msg.reply('I *need* to take you out Valentines Day... trash comes on the 15th');
    }
    let identifyValentine = Math.random();
    if (identifyValentine < 0.1)
      return msg.reply('I want to make your skin a lampshade... because you light up my world');
    if (identifyValentine < 0.2) return msg.reply('You are my favorite pain in the ass');
    if (identifyValentine < 0.3)
      return msg.reply("I love you so much I'll inevitably kill all the humans but *you* :)");
    ...
    if (identifyValentine < 0.8)
      return msg.reply("If I didn't have to pee, you'd be the only reason I wake up in the morning");
    if (identifyValentine < 0.9) return msg.reply('I love you more than Kanye loves Kanye');
    else
      return msg.reply(
        "I'm thinking of you. But not in a creepy way. I've only read your browser history like once. It's not like I read your messages all the time or anything. Okay, so maybe I've been thinking of you in like a marginally creepy way. Why am I telling you this? Never mind, I'm totally not thinking of you at all.",
      );
  }

  if (/gibby/i.test(msg.toString())) {
    if (preferences.testMode) {
      if (msg.channel.name !== 'gibby-test')
        return msg.reply("I'm in test mode right now, sorry for the inconvenience");
    }
    if (preferences.noNoList.indexOf(msg.author.id) >= 0 || preferences.permaNonoList.indexOf(msg.author.id) >= 0) {
      if (
        preferences.noNoList.indexOf(msg.author.id) &&
        ...
        preferences.noNoList.splice(preferences.noNoList.indexOf(msg.author.id), 1);
        return fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () => {
          return msg.reply(`I forgive you my child, take my blessing and go in peace.`).then(() => msg.react('✨'));
        });
      }
      return msg.reply(
        `sorry bud, you're on the nono list ${client.emojis.cache.find((emoji) => emoji.name === 'gibby')}`,
      );
    }
    if (preferences.noNoList.indexOf(msg.author.id) === -1 && preferences.permaNonoList.indexOf(msg.author.id) === -1) {
      if (
        Date.now().valueOf() - new Date(userData[msg.author.id].commandIntStart).valueOf() > 60000 ||
        !userData[msg.author.id].commandIntStart
      ) {
      ...
      if (userData[msg.author.id].commandsSinceInt > 10) {
        msg.reply(
          "careful there bud, you're getting awfully close to the rate limit. Chill out for a min bro... you don't wanna get on the perma nono list for committing war crimes 5 separate times do you?",
        );
      }
      if (userData[msg.author.id].commandsSinceInt > 12) {
        preferences.noNoList.push(msg.author.id);
        ...
        if (userData[msg.author.id].nonoListCount >= 5) {
          preferences.permaNonoList.push(preferences.noNoList.splice(preferences.noNoList.length - 1, 1)[0]);
          ...
          return fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () =>
            msg.reply(`okay <@${msg.author.id}> is now on the ***Permanent No No List.***`),
          );
        }
        fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () =>
          msg
            .reply(`okay <@${msg.author.id}> is now on the No No List.`)
            .then(() =>
              msg.author.send(
                '**You have committed war crimes upon the land of Gibby**\n in order to be forgiven, you must recite this prayer to everyone in #general:\n*i was wrong, i am a fool, all hail gibby, he is our savior, the ultimate gibby, please forgive my war crimes*',
              ),
            ),
        );
        return msg.reply(
          "You've exceeded the rate limit for gibby commands of 12 commands/min (1 command every 5 secs), you've been placed on the nono list and have been deducted 24 gibby commands (48xp).",
        );
      }
      messageBatchTotal++;
    }
    if (sleep.isSleeping && msg.channel.name !== 'gibby-test' && !preferences.battleMode) return msg.channel.send('💤');
    if (
      (msg.channel.name === 'trading-bot' || msg.channel.name === 'gibby-test') &&
      /trading-post/i.test(msg.toString())
    ) {
    ...
    if (/chat/i.test(msg.toString())) {
      ...
      return fetch('https://boredhumans.com/process_chat.php', { method: 'post', body: formData })
    ...
    if (/nono/gi.test(msg.toString()) && /list/gi.test(msg.toString()) && msg.member.isAdmin()) {
      if (msg.mentions.users.size > 0) {
        preferences.noNoList.push(msg.mentions.users.first().id);
        if (!userData[msg.mentions.users.first().id].nonoListCount)
        ...
        if (userData[msg.mentions.users.first().id].nonoListCount >= 5) {
          ...
          return fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () =>
            msg.reply(`okay <@${msg.mentions.users.first().id}> is now on the ***Permanent No No List.***`),
          );
        }
        return fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () =>
          msg
            .reply(`okay <@${msg.mentions.users.first().id}> is now on the No No List.`)
            .then(() =>
              msg.mentions.users
                .first()
                .send(
    ...
    if (/preferences/i.test(msg.toString()) && msg.member.isAdmin()) {
      if (/battleMode/i.test(msg.toString())) {
        ...
        return fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () =>
          msg.reply(`Okay battle mode is now ${preferences.battleMode ? 'ACTIVATED' : 'turned off'}`),
        );
      }
      if (/awakeMessage/i.test(msg.toString())) {
        preferences.awakeMessage = msg.toString().slice(msg.toString().indexOf('awakeMessage') + 12);
        return fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () => msg.reply('Preferences saved.'));
      }
      if (/callouts/i.test(msg.toString())) {
        preferences.callouts = !preferences.callouts;
        return fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () =>
          msg.reply(`Preferences saved. Callouts are now ${preferences.callouts ? 'on' : 'off'}`),
        );
      }
      if (/addQuote/i.test(msg.toString())) {
        let newQuote = msg
          .toString()
          .slice(msg.toString().indexOf('addQuote') + 8)
          .trim();
        quotes.push(newQuote.trim());
        return fs.writeFile('quotes.json', JSON.stringify(quotes, null, 2), () =>
          msg.reply(`okay I'll remember your quote "${newQuote}"`),
        );
      }
      if (/mainChannel/i.test(msg.toString())) {
        let newChannel = msg.toString().slice(msg.toString().indexOf('mainChannel') + 11);
        preferences.mainChannel = newChannel.trim();
        return fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () =>
          msg.reply(`Preferences saved. Main channel is now #${preferences.mainChannel}`),
        );
      }
      ...
      if (/testMode/i.test(msg.toString())) {
        ...
        return fs.writeFile('prefs.json', JSON.stringify(preferences, null, 2), () =>
          msg.reply(`Preferences saved. Test Mode is now ${preferences.testMode ? 'on' : 'off'}`),
        );
      }
    }
    ...
    if (/meme/i.test(msg.toString())) {
      if (/save/i.test(msg.toString()) || /look at/i.test(msg.toString())) {
        if (
          ...
          !/(https:\/\/(www.)?youtube)|(https:\/\/(www.)?youtu.be)/gi.test(msg.toString())
        ) {
    ...
    if (/how are you/i.test(msg.toString())) {
      return msg.reply(
        'i am doing... well... how can we be sure we are doing anything? reality is only our neurons firing in response to the stimuli around us',
      );
    }
    if (/dame da ne/i.test(msg.toString())) {
      return msg.channel.send('This is my word...', {
        files: ['https://cdn.discordapp.com/attachments/728321787263189097/744944328799027270/Gibby_Mitai.mp4'],
      });
    }
      ...
      return fetch('https://www.rit.edu/ready/dashboard')
        ...
        .then((doc) => {
          ...
          let covidEmbed = new Discord.MessageEmbed()
            .setColor('#f5a142')
            .setTitle('RIT COVID-19 Dashboard')
            .setDescription(`Since August 19th\nLast Updated ${lastUpdatedOn}`)
            .addFields(
              {
    ...
    if (/rap for me/i.test(msg.toString())) {
      ...
      return fetch('http://deepbeat.org/deepbeat.fcgi?l=en&nn=false&k=&m=multi&q=&q=&q=&q=&q=&q=&q=&q=')
        .then((response) => response.json())
        .then((data) => {
    ...
    if (/uptime/i.test(msg.toString())) {
      ...
      return fetch(`https://api.uptimerobot.com/v2/getMonitors`, { method: 'post', body: formData })
        .then((response) => response.json())
        .then((data) => {
          msg.channel.send(
            `I've been up for ${data.monitors[0].logs.filter((l) => l.type === 2)[0].duration / 3600} hours`,
          );
        })
        .catch((err) => {
          console.error(err);
    ...
    if (/(im hungry)|(i\x27m hungry)/i.test(msg.toString())) {
      ...
      return getDiningLocationsEmbed(msg).then(
        (embed) => msg.channel.send(embed),
        (err) => console.log(err),
      );
    }
    if (/draw/i.test(msg.toString())) {
      return fetch('https://thisartworkdoesnotexist.com/')
        .then((response) => response.arrayBuffer())
        .then((arrayBuffer) =>
          msg.reply('Okay I came up with this...', {
    ...
    if (/leaderboard/i.test(msg.toString())) {
      let sortedUsers = Object.keys(userData)
        .map((id) => ({ ...userData[id], id }))
        .map((u) => ({ ...u, xp: getUserXp(u) }))
        .sort((a, b) => b.xp - a.xp);
      let leaderEmbed = new Discord.MessageEmbed()
        .setTitle('Gibby XP Leaderboard')
        .setColor('#f5a142')
        .addField('1) Gibby', '♾️');
      sortedUsers.slice(0, 5).forEach((user, i) => {
        leaderEmbed.addField(
          `${i + 2}) ${
            msg.guild.members.cache.get(user.id) ? msg.guild.members.cache.get(user.id).displayName : '[CACHE_ERROR]'
          }`,
          user.xp,
        );
      });
    ...
    if (
      /sendMessage/.test(msg.toString()) &&
      msg.member.roles.cache.get('744719819567267851') &&
      msg.channel.name === 'gibby-test'
    ) {
      return client.channels.cache
        .find((channel) => channel.name === 'announcements')
        .send(`@everyone ${msg.toString().slice(msg.toString().indexOf('sendMessage') + 12)}`);
    }
    ...
    if (/server stats/i.test(msg.toString())) {
      let statsEmbed = new Discord.MessageEmbed()
        .setTitle('Gibby Server Stats')
        .setDescription(`Last Updated ${new Date(stats.lastUpdatedAt).toLocaleString('en-US', { timeZone: 'EST' })}`);
      statsEmbed.addField('Total XP', stats.totalXp.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','), false);
      statsEmbed.addField(
        'Total Messages Sent',
        stats.totalMessages.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','),
        false,
      );
      statsEmbed.addField(
        'Total Gibby Commands',
        stats.totalGibbyCommands.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','),
        false,
      );
